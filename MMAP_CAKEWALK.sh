...
#create covariate file for LZ analysis
if [ $format -eq 0 ]
then
	cp covar.txt covar.LZ.txt
else	
	#switch row to column
	sed 's/ \+/,/g' covar.txt > top_row.csv
	tr -s ',' '\n' < top_row.csv | tr -d '"' > top_col.csv
	...
	#add to list
	awk -v a=$a -F"," 'NR==a {print $2}' TOP_SNPS.csv >> top_col.csv
	awk 'BEGIN {ORS = ","}{print}' top_col.csv > covar_new.csv
	sed 's/\,/\t/g' covar_new.csv > cov_new.txt
	sed 's/ /\	/g' cov_new.txt > covar.LZ.txt
fi	
...
do
	...
	covariates_LZ=`awk -Ft 'NR==1 {print}' covar.LZ.txt`
	...
	$locuszoom --build hg38 --epacts $epacts --ld $txtld --refsnp $LZ_refsnp --chr $chr --start $min --end $max geneFontSize=.5 ymax=$ymax vertline="$vertline" vertLineColor="green4" vertLineType="1" vertLineWidth="1" signifLine=$sigline signifLineColor="red,royalblue" title="$trait on Chr$chr" titleCex=1.3 title2="Top Variant: $calcLD_refsnp Covariates: $covariates_LZ" title2Cex=1.0 title2Color="gray30" showRecomb=TRUE recombColor="gray80" recombAxisColor="gray60" --plotonly --cache None --no-date --prefix "LZ_$trait.$name.ImpRgn3TM5"  >> LZdialog.log 2>&1 
